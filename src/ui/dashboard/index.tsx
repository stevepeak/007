...
export function WfDashboard({ className }: { className?: string }) {
  ...
  return (
    <QueryState
      ...
      // A dashboard is a nicety on top of the launcher below it, so a failure
      // here stays quiet rather than taking the whole home page down with it —
      // same for no data at all, which `empty` renders as nothing.
      error={() => null}
    >
      {(data) => (
        <div className={className}>
