...
// A comment-preserving pretty-printer. Unlike the compiler, formatting can't go
// through JSON Schema (that would strip the author's comments), so it works on
// the token stream directly: it regenerates all whitespace, expanding
// `z.object({ … })` bodies one-field-per-line and keeping arrays/enums/calls
// inline — matching the decompiler's house style — while carrying comments
// through verbatim, either on their own line or trailing the code they annotate.

type FmtKind = 'name' | 'string' | 'punct' | 'line-comment' | 'block-comment'
type FmtToken = { kind: FmtKind; value: string; nlBefore: boolean }

// Like `tokenize`, but keeps comments as tokens and records whether a newline
// preceded each token (so a comment can be re-attached as leading vs trailing).
...
// ---- Decompiler (JSON Schema → Zod source) ---------------------------------

/**
 * Best-effort inverse of {@link compileZodSource}: render a JSON Schema back to
